...
#include "F2837xD_EQep_defines.h"
...
void eQEP_initialize(int m)
{
    ...
    //  //旋变初始位置采集
    // DELAY_US(11);         //RESET 低电平至少10US
    // GpioDataRegs.GPDSET.bit.GPIO106=1;
    // DELAY_US(30001);      //RESET 高电平至少30MS
    ...
    EQep1Regs.QEINT.bit.UTO=1; //使能定时器的中断
    ...
    //        EQep1Regs.QEPCTL.bit.PCRM=3;//00;   // PCRM=00 mode - QPOSCNT reset on index event
    //        #if USING_UNIT_TIME_INTERRUPT
    ...
    //        EQep1Regs.QPOSMAX = (Uint32)SYSTEM_QEP_PULSES_PER_REV; // 0xFFFF; //0xffffffff;
    //
    //        //EQep1Regs.QCAPCTL.bit.UPPS=5;     // 1/32 for unit position
    //        EQep1Regs.QCAPCTL.bit.UPPS=9;       // 1/512 for unit position
