...
void COMM_resistanceId_v2(REAL id_fb, REAL iq_fb){
    ...
    #define RS_ID_MAXIMUM_CURRENT (1*d_sim.init.IN) /* [A] This specifies beta-axis current, so we need to make it larger so that the phase current is large enough This is motor related (to identify accurate resistance) */
    #ifdef _XCUBE1
...
void StepByStepCommissioning(){
    ...
    if(COMM.bool_comm_status == 0){
        ...
        // 电阻辨识
        if(1){/* WUBO is an ASSHOLE cuz he makes it 1*/
