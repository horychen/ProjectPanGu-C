...
void Modified_ParkSul_Compensation(void)
{
    ...
    /* TO use this, you muast have a large enough stator current */
    /* TO use this, you muast have a large enough stator current */
    INV.Vsat += CL_TS * INV.gain_Vsat * sinf(ENC.theta_d_elec - ELECTRICAL_POSITION_FEEDBACK) * sign(ENC.varOmega);
    if (INV.Vsat > 15)
    {
        INV.Vsat = 15;
    }
...
void Online_PAA_Based_Compensation(void)
{
    ...
    {
        INV.thetaA = -M_PI * 1.5 + ELECTRICAL_POSITION_FEEDBACK + atan2((*CTRL).i->cmd_iDQ[1], (*CTRL).i->cmd_iDQ[0]); /* Q: why -pi*(1.5)? */ /* ParkSul2014 suggests to use PLL to extract thetaA from current command */
    }
    else
    {
        INV.thetaA = -M_PI * 1.5 + (*CTRL).i->theta_d_elec + atan2((*CTRL).i->cmd_iDQ[1], (*CTRL).i->cmd_iDQ[0]); /* Q: why -pi*(1.5)? */ /* ParkSul2014 suggests to use PLL to extract thetaA from current command */
