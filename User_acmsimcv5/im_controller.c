...
REAL controller(REAL set_rpm_speed_command,
    ...
    static REAL rpm_speed_command=0.0, amp_current_command=0.0;
    {
        ...
        if((*CTRL).timebase>6){/*Variable Speed: Sinusoidal Speed + Ramp Speed*/
            ...
            (*CTRL).i->cmd_varOmega        = (SINE_AMPL          * sin(OMG1*(*CTRL).timebase) + local_dc_rpm_cmd       )*RPM_2_ELEC_RAD_PER_SEC;
            (*CTRL).i->cmd_deriv_varOmega  = (SINE_AMPL*OMG1     * cos(OMG1*(*CTRL).timebase) + local_dc_rpm_cmd_deriv )*RPM_2_ELEC_RAD_PER_SEC;
            (*CTRL).i->cmd_dderiv_varOmega = (SINE_AMPL*OMG1*OMG1*-sin(OMG1*(*CTRL).timebase) + 0                      )*RPM_2_ELEC_RAD_PER_SEC;
        }else if((*CTRL).timebase>5+OFF){/*Constant Speed*/
            rpm_speed_command           = local_dc_rpm_cmd;
            (*CTRL).i->cmd_varOmega        = rpm_speed_command*RPM_2_ELEC_RAD_PER_SEC;
            ...
            (*CTRL).i->cmd_varOmega        = 0; // 0*20                   * sin(2*M_PI*(*CTRL).timebase)*RPM_2_ELEC_RAD_PER_SEC;
            (*CTRL).i->cmd_deriv_varOmega  = 0; // 0*20*(2*M_PI)          * cos(2*M_PI*(*CTRL).timebase)*RPM_2_ELEC_RAD_PER_SEC;
            (*CTRL).i->cmd_dderiv_varOmega = 0; // 0*20*(2*M_PI)*(2*M_PI) *-sin(2*M_PI*(*CTRL).timebase)*RPM_2_ELEC_RAD_PER_SEC;
        }
    }
    ...
    if(CONSTANT_SPEED_OPERATION!=0){
        rpm_speed_command           = 0;
        (*CTRL).i->cmd_varOmega        = CONSTANT_SPEED_OPERATION*RPM_2_ELEC_RAD_PER_SEC;
        (*CTRL).i->cmd_deriv_varOmega  = 0;
        (*CTRL).i->cmd_dderiv_varOmega = 0;
    }
...
void controller_marino2005(){
    ...
    if(TRUE){
        ...
        marino.deriv_iQ_cmd =   (*CTRL).motor->npp_inv*(*CTRL).motor->Js * CLARKE_TRANS_TORQUE_GAIN_INVERSE*(*CTRL).motor->npp_inv * (\
            1.0*(-marino.k_omega*deriv_sat_kappa((*CTRL).i->varOmega-(*CTRL).i->cmd_varOmega) * (marino.deriv_xOmg - (*CTRL).i->cmd_deriv_varOmega) + (*CTRL).motor->Js_inv*(*CTRL).motor->npp*marino.deriv_xTL + (*CTRL).i->cmd_dderiv_varOmega ) * (*CTRL).i->cmd_psi_inv\
          - 1.0*(-marino.k_omega*      sat_kappa((*CTRL).i->varOmega-(*CTRL).i->cmd_varOmega) + (*CTRL).motor->Js_inv*(*CTRL).motor->npp*(*CTRL).i->TLoad + (*CTRL).i->cmd_deriv_varOmega) * ((*CTRL).i->cmd_deriv_psi * (*CTRL).i->cmd_psi_inv*(*CTRL).i->cmd_psi_inv)
            );

        // current error quantities
        (*CTRL).i->cmd_iDQ[0] = ( (*CTRL).i->cmd_psi + (*CTRL).i->cmd_deriv_psi*(*CTRL).motor->alpha_inv ) * (*CTRL).motor->Lmu_inv;
        (*CTRL).i->cmd_iDQ[1] = ( (*CTRL).motor->npp_inv*(*CTRL).motor->Js *( 1*(*CTRL).i->cmd_deriv_varOmega - marino.k_omega*sat_kappa((*CTRL).i->varOmega-(*CTRL).i->cmd_varOmega) ) + (*CTRL).i->TLoad ) * (CLARKE_TRANS_TORQUE_GAIN_INVERSE*(*CTRL).motor->npp_inv*(*CTRL).i->cmd_psi_inv);
        ...
        marino.Gamma_Q = (*CTRL).motor->Lsigma_inv * (-(*CTRL).motor->R*(*CTRL).i->iDQ[1] -(*CTRL).motor->alpha*(*CTRL).motor->Lmu*(*CTRL).i->cmd_iDQ[1] -(*CTRL).i->varOmega*(*CTRL).i->cmd_psi -(*CTRL).s->omega_syn*marino.e_psi_Dmu) -(*CTRL).s->omega_syn*(*CTRL).i->iDQ[0] - marino.deriv_iQ_cmd;
...
void controller_IFOC(){
    ...
    (*CTRL).s->omega_syn = (*CTRL).i->varOmega + (*CTRL).s->omega_sl;
