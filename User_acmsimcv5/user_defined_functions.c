...
void main_switch(){
    ...
    case MODE_SELECT_VOLTAGE_OPEN_LOOP: // 11
        (*CTRL).o->cmd_uAB_to_inverter[0] = 3 * cos(5*2*M_PI* CTRL->timebase); // test
        (*CTRL).o->cmd_uAB_to_inverter[1] = 3 * sin(5*2*M_PI* CTRL->timebase); // test
        break;
